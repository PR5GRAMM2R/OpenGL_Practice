...
int main()
{
    ...
    //glEnable(GL_FRAMEBUFFER_SRGB);  // Gamma 보정하는 방법 1

    //glEnable(GL_CULL_FACE);     // Cull Face 를 활성화 ( depth map 에서 front face cull 을 하기 위함 )


    // tell stb_image.h to flip loaded texture's on the y-axis (before loading model).
    stbi_set_flip_vertically_on_load(true);

    Shader shader("shaders/shadowMappingShader.vs", "shaders/shadowMappingShader.fs");
    Shader depthShader("shaders/depthShader.vs", "shaders/depthShader.fs");
    Shader depthQuadShader("shaders/depthQuadShader.vs", "shaders/depthQuadShader.fs");
    ...
    unsigned int depthMapFBO;               // 깊이 맵을 렌더링하기 위한 프레임 버퍼 객체
    glGenFramebuffers(1, &depthMapFBO);

    const unsigned int SHADOW_WIDTH = 1024, SHADOW_HEIGHT = 1024;

    unsigned int depthMap;                  // 깊이 맵을 생성
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO); // 광원 관점에서 렌더링할 시 깊이 정보는 필요하지만 색상 버퍼는 필요하지 않음
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthMap, 0);
    glDrawBuffer(GL_NONE);                  // 따라서 OpenGL 에게 색상 버퍼를 통한 렌더링을 할 필요가 없다고 명시함
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        lightProjection = glm::perspective<float>(glm::radians(70.0f), 1.0f, near_plane, far_plane);

...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {	// GLFW 가 종료되도록 (창을 닫도록) 지시받았는지 확인
		...
		glm::mat4 view = camera.GetViewMatrix();	// 카메라의 위치 변화에 따른 시점 이동

		glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 1000.0f);	// 카메라의 시야 변화
